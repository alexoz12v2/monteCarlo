...
una distribuzione\footnotemark{} che trasforma Irradianza incidente in Radianza riflessa o trasmessa.\par 
Nell'ipotesi che il materiale sia a regime, senza effetti di fosforescenza e 
fluorescenza (restringendoci all'ottica geometrica\footnotemark{}), possiamo modellare il materiale con una densit\`a di distribuzione che modella 
la tendenza del materiale alla riflessione e/o trasmissione della luce incidente, e in quali direzioni, trascurando il fenomeno di 
subsurface scattering e supponendo che essa fuoriesca esclusiamente dal punto di incidenza 
(dunque soffermandoci soltanto sui fenomeni a livello dell'interfaccia). Tale distribuzione \`e detta \Gls{BRDF}(BRDF)
\begin{definitionS}
...
\begin{equation}\label{chapter3:surface:reflectedDirection}
...
allora la direzione di trasmissione
{
	...
	\begin{align}\label{chapter3:surface:refractedDirection}
...
\footnotetext{\textit{Importance Sampling}}
\begin{equation}
...
\end{equation}
da cui
\begin{equation}\label{chapter3:surface:lambert}
...
\begin{align}\label{chapter3:surface:cosineLobeSampling}
...
\begin{equation}\label{chapter3:BRDF:specular}
...
\par
Analizziamo brevemente un modello di visibilit\`a, la cui dimostrazione completa si rimanda a \cite{pegoraro}: \textit{V-Cavity Function}.
...
\end{equation}
Si pu\`o dimostrare (\cite{pegoraro}) che tali assunzioni conducono alla seguente BRDF, diffusissima nelle applicazioni moderne
\begin{equation}\label{chapter3:surface:cookTorrance}
...
\end{equation}
Seguendo gli stessi passaggi, possiamo ottenere espressioni analoghe per la BTDF
\newcommand{\htoi}{\langle\hat{h}_t,\hat{\omega}_i\rangle}
\newcommand{\htoo}{\langle\hat{h}_t,\hat{\omega}_o\rangle}
\begin{align}
	\hat{h}_t&=\operatorname{sgn}\left(1-\frac{\eta_i}{\eta_t}\right)\frac{\eta_t\hat{\omega}_o+\eta_i\hat{\omega}_i}
		{\norm{\eta_t\hat{\omega}_o+\eta_i\hat{\omega}_i}}\\
	f_\mu(\hat{m},\hat{\omega}_o,\hat{\omega}_i)&=\frac{\delta(\hat{h}_t-\hat{m})}{-\langle\hat{m},\hat{\omega}_i\rangle}
		\frac{-\eta_t^2\htoi T_{\frac{\eta_i}{\eta_t}}(\langle\hat{m},\hat{\omega}_o\rangle)}%
			{(\eta_t\htoo+\eta_i\htoi)^2}\\
	f_t(\vec{p},\hat{\omega}_o,\hat{\omega}_i)&=\frac{D(\vec{p},\hat{h}_t)G(\vec{p},\hat{h}_t,\hat{\omega}_o,\hat{\omega}_i)}
...
%	\end{align}
%}
Si analizzano di seguito due alternative di NDF, anello mancante del modello presentato.
